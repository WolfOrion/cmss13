...
/datum/equipment_preset/survivor/forecon/proc/add_forecon_weapon(mob/living/carbon/human/new_human)
	...
	switch(random_gun)
		if(1 , 2)
			new_human.equip_to_slot_or_del(new /obj/item/weapon/gun/rifle/m41a(new_human), WEAR_R_HAND)
			new_human.equip_to_slot_or_del(new /obj/item/ammo_magazine/rifle(new_human), WEAR_IN_BACK)
		if(3 , 4)
			new_human.equip_to_slot_or_del(new /obj/item/weapon/gun/rifle/m4ra(new_human), WEAR_R_HAND)
...
/datum/equipment_preset/survivor/forecon/proc/add_forecon_equipment(mob/living/carbon/human/new_human)
	...
	switch(random_equipment)
		if(1)
			new_human.equip_to_slot_or_del(new /obj/item/device/walkman(new_human), WEAR_R_EAR)
			new_human.equip_to_slot_or_del(new /obj/item/device/cassette_tape/indie(new_human), WEAR_IN_ACCESSORY)
		if(2)
...
/datum/equipment_preset/survivor/forecon/proc/add_forecon_gloves(mob/living/carbon/human/new_human)
...
/datum/equipment_preset/survivor/forecon/proc/spawn_random_headgear(mob/living/carbon/human/new_human)
	...
	switch(i)
		...
		if (8)
			new_human.equip_to_slot_or_del(new /obj/item/clothing/head/beret/marine/mp/mpcap/delta(new_human), WEAR_HEAD)
		if (9)
...
/datum/equipment_preset/survivor/forecon/sniper/load_gear(mob/living/carbon/human/new_human)
	...
	var/obj/item/clothing/accessory/patch/patch_uscm = new()
	var/obj/item/clothing/accessory/patch/forecon/patch_forecon = new()
	var/obj/item/clothing/accessory/poncho/poncho = new()
	var/obj/item/clothing/suit/storage/marine/ghillie/jacket = new()
	uniform.attach_accessory(new_human,pouch)
	uniform.attach_accessory(new_human,patch_uscm)
	uniform.attach_accessory(new_human,pin)
	uniform.attach_accessory(new_human,patch_forecon)
	jacket.attach_accessory(new_human,poncho)
	new_human.equip_to_slot_or_del(uniform, WEAR_BODY)
...
/datum/equipment_preset/survivor/forecon/major/load_gear(mob/living/carbon/human/new_human)
	...
	var/obj/item/clothing/suit/storage/windbreaker/windbreaker_green/jacket = new()
	var/obj/item/clothing/accessory/storage/droppouch/pouch = new()
//	var/obj/item/clothing/accessory/ranks/marine/o4/pin = new()
	...
	new_human.equip_to_slot_or_del(new /obj/item/ammo_magazine/revolver/mateba/highimpact(new_human), WEAR_IN_BELT)
	new_human.equip_to_slot_or_del(new /obj/item/ammo_magazine/revolver/mateba/highimpact(new_human), WEAR_IN_BELT)
	new_human.equip_to_slot_or_del(new /obj/item/clothing/mask/cigarette/cigar(new_human), WEAR_IN_ACCESSORY)
	new_human.equip_to_slot_or_del(new /obj/item/tool/lighter/zippo/gold(new_human), WEAR_IN_ACCESSORY)
	new_human.equip_to_slot_or_del(new /obj/item/reagent_container/food/drinks/bottle/davenport(new_human), WEAR_IN_JACKET)
...
/datum/equipment_preset/synth/survivor/forecon
	...
	faction_group = list(FACTION_MARINE, FACTION_SURVIVOR)
	idtype = /obj/item/card/id/gold
	role_comm_title = "FORECON Synth"

/datum/equipment_preset/synth/survivor/forecon/load_gear(mob/living/carbon/human/preset_human) //Bishop from Aliens
	preset_human.equip_to_slot_or_del(new /obj/item/clothing/under/marine/officer/engi(preset_human), WEAR_BODY)
	...
	preset_human.equip_to_slot_or_del(new /obj/item/clothing/accessory/patch/forecon(preset_human), WEAR_ACCESSORY)
	preset_human.equip_to_slot_or_del(new /obj/item/device/flashlight(preset_human), WEAR_R_HAND)
	preset_human.equip_to_slot_or_del(new /obj/item/device/motiondetector(preset_human), WEAR_L_HAND)
	preset_human.equip_to_slot_or_del(new /obj/item/storage/pouch/tools/tactical/full(preset_human), WEAR_R_STORE)
	preset_human.equip_to_slot_or_del(new /obj/item/storage/pouch/medkit/full_advanced(preset_human), WEAR_L_STORE)
	preset_human.equip_to_slot_or_del(new /obj/item/storage/firstaid/surgical(preset_human.back), WEAR_IN_BACK)
	preset_human.equip_to_slot_or_del(new /obj/item/device/defibrillator(preset_human), WEAR_IN_BACK)
